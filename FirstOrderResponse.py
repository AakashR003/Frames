...
class FirstOrderGlobalResponse(Model):
    ...
    def EigenAnalysis(self, Solver = "eigs"):
        StiffnessMatrix = self.GlobalStiffnessMatrixCondensed()
        CriticalLoad , EigenMode = eig(StiffnessMatrix)
        EigenMode = Computer.EigenSolver(StiffnessMatrix, Solver = Solver, k=len(self.UnConstrainedDoF()))
    ...
    def MemberEigenMode(self, MemberNumber, scale_factor = 1, EigenModeNo = 1, EigenVectorDict = None):
        ...
        if EigenVectorDict == None:
            EigenVector = self.EigenAnalysis(EigenModeNo = EigenModeNo)[2][:, (EigenModeNo-1)]
    ...
    def CalculateModalStiffness(self):

        SM = self.GlobalStiffnessMatrixCondensed()
        OrthogonalStiffnessMatrix = Computer.OrthogonalSolver(SM, SMatrix= SM)
        row_max_abs = np.max(np.abs(OrthogonalStiffnessMatrix), axis=1)

        return row_max_abs
    
    def OrthogonalForceVector(self):
        """
        Returns the critical force points for the model for each eigenmode
        """
        #todo: Circular Import is not checked - NeumanBC

        Force_Vector = np.array(self.ForceVector())
        OrthogonalForceVector = Computer.OrthogonalSolver(Force_Vector, SMatrix=self.GlobalStiffnessMatrixCondensed())

        abs_with_index = [(i, abs(val)) for i, val in enumerate(OrthogonalForceVector)]
        # Sort by abs value descending
        abs_with_index.sort(key=lambda x: x[1], reverse=True)

        print("Contribution, Mode",abs_with_index)

        return OrthogonalForceVector
    ...
    def PlotEigenMode(self, EigenModeNo = 1, scale_factor = 1, Solver ="eigsh", show_structure = True):
        ...
        Eigen = self.EigenAnalysis(Solver = Solver)
        ...
        ax.set_title(f"Linear Eigen Mode {EigenModeNo} - {Eigen[1][EigenModeNo-1]}")
        ax.axis('equal')
        plt.show()
